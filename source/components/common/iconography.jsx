 ...
 */
export function Icon({className, icon, iconClass = "material-symbols-rounded", __IS_IN_BUTTON = false, ...props}) {
    const {containerContext} = useContainer();
    const {role, type} = containerContext();
    const {cx, classes} = useStyles({role, type, isInButton: __IS_IN_BUTTON});
    return (
 ...
 */
export default function IconButton({className, role = "primary", appearance = "filled", icon, iconClass = "material-symbols-rounded", onMouseDown, onMouseUp, ...props}) {
